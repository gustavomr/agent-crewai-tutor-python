...
def process_zip_file(zip_path):
    ...
    try:
        ...
        # Filter out api_fase01.py and other support files
        python_files = [f for f in python_files if not any(exclude in os.path.basename(f).lower() 
                        for exclude in ['api_fase01.py', 'api_call.py', 'api_client.py', 'main.py'])]
        
        # Combine both file types
        all_files = python_files + notebook_files
        
        if not all_files:
            print("❌ No valid Python (.py) or Jupyter notebook (.ipynb) files found in the zip archive (excluding support files).")
            return None, temp_dir
        
        # Try to find the main student file (prefer files that don't look like support files)
        # Priority: files with "main", "projeto", "fase", or the student's name
        student_files = []
        for file_path in all_files:
            filename = os.path.basename(file_path).lower()
            # Prioritize files that seem to be the main submission
            if any(keyword in filename for keyword in ['main', 'projeto', 'fase', 'trabalho', 'coleta']):
                student_files.append(file_path)
        
        # If no priority files found, use the first valid file
        if student_files:
            file_path = student_files[0]
            print(f"✅ Found main student file: {os.path.basename(file_path)}")
        else:
            file_path = all_files[0]
            print(f"✅ Found Python file: {os.path.basename(file_path)}")
...
def analyze_student_code(codigo_aluno, student_name, llm):
    ...
    tarefa_analise = Task(
        ...
        1. Valida se o código é organizado, não existem importações no meio do código e se existe comentários? (Responda com Sim ou Não)
        2. Coleta dados da API REST. (Responda com Sim ou Não)
        3. Usou pandas para coletar dados do CSV. (Responda com Sim ou Não)
        4. Usou alguma função do pandas para concatenar os dados da API REST com os dados do CSV. (Responda com Sim ou Não)
        5. O arquivo CSV com os dados concatenados foi gerado e enviado. (Responda com Sim ou Não)
        
        Lembre-se que o código pode estar dentro de um arquivo .py ou .ipynb. O arquivo que você deve olhar não é o api_fase01.py.
